...
- [ ] Propagate per-post metadata: pass `description` and `image` from `src/pages/writing/[slug].astro` → `PostLayout` → `BaseLayout` and use them in head meta (`og:description`, `og:image`, `x:description`, `x:image`).
